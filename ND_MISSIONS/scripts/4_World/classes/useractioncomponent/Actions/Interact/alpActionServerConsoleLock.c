class alpActionServerConsoleLock: ActionInteractBase
{
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item ){
		...
		if ( GetGame().IsClient() ) {
			...
			if ( console && console.CanBeLockedALP() && console.IsUnlockedALP() && !console.IsRuinedALP() && !console.IsDamagedALP() && card && card.GetKeyID()==console.GetAccesCardID()) {
