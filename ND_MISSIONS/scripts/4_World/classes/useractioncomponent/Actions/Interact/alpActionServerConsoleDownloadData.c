...
class alpActionServerConsoleDownloadData extends ActionInteractLoopBase
{
	...
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item ){
		...
		if ( GetGame().IsClient() ) {
			...
			if ( console && console.IsUnlockedALP() && !console.IsRuinedALP() && !console.IsDamagedALP() && console.IsSetSettingALP(alpMISSIONTRIGGER.IS_DATA_PANEL) && !console.IsSetSettingALP(alpMISSIONTRIGGER.IS_DATA_DELETED) ) {
				return true;
			}
	...
	override void OnEndServer( ActionData action_data )
	{
		...
		if ( console ) {
			console.DownloadData(action_data.m_Player);	
		}			
	}
